...
### 🔧 修复
- fix(avatar): 肘部穿透修正不完整——上臂旋转未随肘部位置同步重算——`ClipBuilder.ts` 的 `solveArmQuaternions` 解析法路径检测到肘部穿入躯干后只修正 `correctedElbowPos` 用于前臂方向计算，但 `upperQuat`（上臂旋转）仍基于未修正的 `upperArmDir`，导致上臂骨骼仍指向穿入躯干的方向；FABRIK 路径则只检测 `elbowPenetratedFabrik` 标志完全不修正；修复方案：(1) 解析法路径在 `if (elbowPenetrated)` 块中用 `correctedElbowPos` 重算 `upperArmDir` 与 `upperQuat`，后续 `invUpper`/`forearmLocalDir`/`lowerQuat` 自动基于修正值，`constrainShoulderByDirection` 自动应用；(2) FABRIK 路径在穿透时用 `projectToSurface` 修正肘部位置，重算 `upperQuatFabrik`/`lowerQuatFabrik` 并应用肩关节约束；(3) 导出 `solveArmQuaternions` 和 `ClipBuildContext` 用于直接测试，新增穿模修正单元测试（构造 5 个负 z 值手腕目标触发穿透，验证修正后上臂方向不穿入躯干）；821 tests passed
...
### 📦 维护
- chore(cleanup): 代码库整洁度打磨——(1) `Smoother.ts` 删除无任何调用方的 `slerpRotation` 函数（@deprecated 且实际未使用），修正 `BoneSmoother` 类的 `@deprecated` 标注（该类仍被 `Avatar3D.tsx` 使用，并非废弃，注释改为说明保留用途）；(2) `types/avatar.ts` 修正 `BonePose` 接口的 `@deprecated` 标注（被 100+ 处引用，仍服务于 2D/skeleton 模式，注释改为"2D/skeleton 模式使用的姿态结构"）；(3) `config.ts` 顶部注释补全 `VITE_POSE_MODEL_URL` 和 `VITE_HAND_MODEL_URL` 两行（与 `appConfig` 实际读取的 8 个环境变量对齐）；(4) `vitest.config.ts` coverage 阈值从 50% 提升到基于实际基线的 57/77/77/57（statements/lines 57%、branches/functions 77%，实际覆盖率 62.59/82.13/82.6/62.59，留 5%+ 缓冲）

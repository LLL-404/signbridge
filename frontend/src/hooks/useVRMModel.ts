// VRM 虚拟人模型 Hook
// 封装 VRM 加载、约束计算、实时驱动、PoseEstimator 访问的全部逻辑
// 供 VRMModel 组件调用，让组件专注于 JSX 渲染
...
const log = logger.module('useVRMModel');
...
function setNeutralPose(vrm: VRM, logFlag = false): void {
  ...
  for (const [name, euler] of [...upperArmSpec, ...lowerArmSpec]) {
    const bone = vrm.humanoid.getNormalizedBoneNode(name) as unknown as {
      quaternion?: { setFromEuler: (e: THREE.Euler) => void };
      rotation?: { setFromEuler: (e: THREE.Euler) => void };
    } | null;
...
/** useVRMModel 配置项（与 VRMModelProps 中渲染相关字段一致） */
export interface UseVRMModelOptions {
...
/** useVRMModel 返回值 */
export interface UseVRMModelReturn {
  /** group ref，组件需将其挂载到 <group> 上以承载 VRM scene */
...
/**
 * VRM 虚拟人模型 Hook
 * - 内部管理 VRM 加载、约束缓存、中立姿态设置
 * - 创建 VRMAnimator / VRMAdapter / RealtimePoseDriver 实例
 * - 通过 useFrame 驱动每帧更新（实时追踪 / 离线动画两条路径）
 * - 通过 ref 同步最新 props，避免 useFrame 闭包过期
 *
 * 注意：本 Hook 内部使用 useFrame，必须在 R3F <Canvas> 内的组件中调用。
 */
export function useVRMModel(options: UseVRMModelOptions = {}): UseVRMModelReturn {
  const {
  ...
  useEffect(() => {
    ...
    loadVRM(modelUrl)
      .then((vrm) => {
        ...
        // useVRMModel.useFrame 调用 vrmAnimator.update(delta) 推进
